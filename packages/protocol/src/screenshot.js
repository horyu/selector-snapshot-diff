...
export const SCREENSHOT_COLOR_SCHEMES = ['light', 'dark', 'no-preference'];

const clampViewport = (value) =>
...
const colorSchemeSchema = z
  ...
  .transform((value, ctx) => {
    if (value === undefined) return undefined;
    if (!SCREENSHOT_COLOR_SCHEMES.includes(value)) {
      ctx.addIssue({
        code: 'custom',
...
export function parseScreenshotPayload(input) {
